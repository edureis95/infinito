					...
					<tr>
	  					...
	  					<td class="">
	  						<select disabled class="form-control input-sm userSelect">
								<option value="{{Auth::user()->id}}">{{Auth::user()->sigla}}</option>
	  						</select>
	  					</td>
	  				</tr>
	  				<tr>
			...
			<table class="table approvalTable smallFontTable">
				...
				@foreach($tasks as $task)
					<tr class="text-center">
						...
						<td>{{$task->t_date}}</td>
						<td>Área</td>
						<td class="text-left">{{sprintf('%05d', $task->p_number)}} - {{$task->p_name}}</td>
						<td>{{$task->e_sigla}}</td>
						<td>{{$task->ph_sigla}}</td>
						<td class="text-left openTaskLayer buttonCursor">{{$task->t_name}}</td>
...
$('.openTaskLayer').click(function() {
	...
	$.ajax({
      ...
      success: function(response) {
			...
			$('.overlayUser').text('{{Auth::user()->name}}');
