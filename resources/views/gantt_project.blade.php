		...
		$( function() {
			...
			gantt.attachEvent("onBeforeTaskDisplay", function(id, task){
				...
				if(task.type == 4) {
					...
					if((task.parent == {{$project->commercial_project_Task_ID}} || task.parent == {{$project->operational_project_Task_ID}} || task.parent == {{$project->plannedTask_id}}) && gantt.hasChild(id)) {
					...
					else if((gantt.getParent(task.parent) == {{$project->commercial_project_Task_ID}} || gantt.getParent(task.parent) == {{$project->operational_project_Task_ID}} || gantt.getParent(task.parent) == {{$project->plannedTask_id}}) && gantt.hasChild(id))
						return true;
					else
						return false;
					...
					if((task.parent == {{$project->commercial_project_Task_ID}} || task.parent == {{$project->operational_project_Task_ID}} || task.parent == {{$project->plannedTask_id}})) {
							return true;
					} else {
						var parent1 = gantt.getParent(task.parent);
						if(parent1.type == 0 || parent1.type == 3 || parent1.type == 7)
							return false;
						if((parent1 == {{$project->commercial_project_Task_ID}} || parent1 == {{$project->operational_project_Task_ID}} || parent1 == {{$project->plannedTask_id}})) {
								return true;
						}
						else {
							...
							if(parent2.type == 0 || parent2.type == 3 || parent2.type == 7)
								return false;
							if((parent2 == {{$project->commercial_project_Task_ID}} || parent2 == {{$project->operational_project_Task_ID}} || parent2 == {{$project->plannedTask_id}})) {
									return true;
							}
						}
					}
			...
			gantt.templates.task_cell_class = function(item, date) {
				...
				@foreach($companyDays as $day)
					if('{{$day->start_date}}' == (date.getFullYear() + '-' + ("0" + month).slice(-2) + '-' + ("0" + date.getDate()).slice(-2))) {
						return 'companyDay';
					}
				@endforeach
