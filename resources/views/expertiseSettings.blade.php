		...
		<div class="panel-body">
			...
			<tbody>
				@foreach($expertise as $expert) 
					<tr class="expert" id="expert{{$expert->id}}">
						@if($expert->parent == 0)
						<td class="code" content="{{$expert->id}}">{{$expert->code}}</td>
						<td class="sigla" content="{{$expert->id}}">{{$expert->sigla}}</td>
						<td class="name" content="{{$expert->id}}">{{$expert->name}}</td>
						...
						<td data-editable='false' class="hidden hiddenForm" style="padding: 0">
							<form action="/settings/addSubExpertise" method="POST">
								...
								<input type="hidden" name="parent" value="{{$expert->id}}">
								<input type="hidden" name="_token" value="{{ csrf_token() }}">
								<button type="submit" class="btn btn-primary">Inserir</button>
							</form>
						</td>
			...
			<table class="table expertiseTableEditable hidden smallFontTable">
				...
				<tbody>
				@foreach($expertise as $expert) 
					<tr class="expertEditable" id="expertEditable{{$expert->id}}">
						@if($expert->parent == 0)
						<td class="code" content="{{$expert->id}}">{{$expert->code}}</td>
						<td class="sigla" content="{{$expert->id}}">{{$expert->sigla}}</td>
						<td class="name" content="{{$expert->id}}">{{$expert->name}}</td>
						<td><button content='{{$expert->id}}' style="padding: 3px 5px;" class="btn btn-danger removeExpertise" type="button"><i class="glyphicon glyphicon-minus"></i></button></td>
						<td></td>
						@endif
					</tr>
				@endforeach
